...
function refreshAuthUI() {
    ...
    const docSection = document.getElementById('docAnalysisSection');
    if (Auth.isLoggedIn()) {
...
async function showSettingsModal() {
    ...
    _loadSettingsSubCard();
}

// ── Subscription Status Card (Feature 10) ────────────────────────────────────
async function _loadSettingsSubCard() {
    const card = document.getElementById('settingsSubCard');
    if (!card) return;
    try {
        const profile = await ApiClient.get('/profiles/me');
        const tierLabel = document.getElementById('settingsTierLabel');
        const usageEl = document.getElementById('settingsAssessmentUsage');
        const upgradeRow = document.getElementById('settingsUpgradeRow');
        if (tierLabel) {
            const tierMap = { free: 'Free', pro_monthly: 'Pro (Monthly)', pro_annual: 'Pro (Annual)', admin: 'Admin' };
            tierLabel.textContent = tierMap[profile.tier] || profile.tier || 'Free';
        }
        if (usageEl) {
            const used = profile.assessments_this_month ?? 0;
            const limit = profile.tier === 'free' ? 3 : 'Unlimited';
            usageEl.textContent = typeof limit === 'number' ? `${used} / ${limit}` : `${used} (Unlimited)`;
        }
        if (upgradeRow) {
            upgradeRow.style.display = (!profile.tier || profile.tier === 'free') ? 'block' : 'none';
        }
...
async function deleteMyAccount() {
    ...
    try {
        ...
        if (typeof Toast !== 'undefined') Toast.error('Account deleted. You have been signed out.');
        Auth.logout();
        if (typeof refreshAuthUI === 'function') refreshAuthUI();
        closeSettingsModal();
    } catch(err) {
        if (typeof Toast !== 'undefined') Toast.error(err.message || 'Failed to delete account. Please try again.');
    }
}
// ─────────────────────────────────────────────────────────────────────────────
